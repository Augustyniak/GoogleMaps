...
POD_NAME = "GooglePlaces"
GOOGLE_MAPS_POD_NAME = "GooglePlaces"
...
# CORE_BINARY = "{build}/Maps/Frameworks/{name}.framework/Versions/A/{name}"\
#     .format(name="GoogleMapsCore", build=BUILD_DIR)
BASE_BINARY = "{build}/Base/Frameworks/{name}.framework/Versions/A/{name}"\
    .format(name="GoogleMapsBase", build=BUILD_DIR)
LIBTOOL_CMD = ["libtool", "-dynamic", BASE_BINARY, "-weak_framework", "UIKit", "-weak_framework", "Foundation", "-ObjC"]
...
def parse_pod(name):
    ...
    #     if "frameworks" in subspec:
    #         frameworks = frameworks + subspec["frameworks"]
    #     if "libraries" in subspec:
    #         libraries = libraries + subspec["libraries"]

    frameworks = frameworks + ['UIKit']
    # libraries = libraries + ['objc', 'System']
    # print("LIBRARIES \n")
    # print(libraries)
    # print("FRAMEWORKS \n")
    # print(frameworks)
...
def parse_base():
    pods_json = subprocess.check_output(["pod", "spec", "cat", "GoogleMaps"])
    pod = json.loads(pods_json)
    ...
    for subspec in pod["subspecs"]:
        if "name" in subspec:
            if subspec["name"] == "Base":
                frameworks = subspec["frameworks"]
                libraries = subspec["libraries"]
...
def main():
    ...
    base_file_url, base_frameworks, base_libs = parse_base()
    ...
    print(BUILD_DIR)
    shutil.copytree("./Base", BUILD_DIR + "/Base")

    output = "{}/{}_dynamic.dylib".format(BUILD_DIR, "GoogleMapsBase")
    targets = ["x86_64", "i386", "armv7", "armv7s", "arm64"]
    dylibs = [link(target, frameworks + base_frameworks, libs + base_libs) for target in targets]

    print color(u"\u2600\ufe0f Creating dynamic library ...")
    cmd = ["lipo", "-output", output, "-create"] + dylibs
    execute(cmd)

    framework = "{build}/Frameworks/{name}.framework".format(name="GoogleMapsBase",
                                                             build=BUILD_DIR)
    ...
    tarfile_name = base_file_url.rsplit("/", 1)[-1]
    print(tarfile_name)
    tarfile_name = "GoogleMapsBase-2.2.0.tar.gz"
